...
- **Continuous clock-drift correction** — The sink now tracks the source's clock by nudging its resampling ratio a few parts per million, instead of dropping a chunk of audio or inserting silence. Ordinary drift is absorbed inaudibly. Drop/insert remains as a safety net for large excursions such as a network stall. The correction loop's gains are derived from the buffer target and the device rate, so it settles within about two minutes at any supported latency instead of hunting; thanks to the prefill (see Fixed, below), standing latency is already correct well before the correction itself has settled. The active correction is reported in the Logs view.
...
- The clock-drift monitor's 2 ppm reporting threshold was sized for a loop that settles on a single number. Under a push/pop pattern close to periodic, sampling the trim at its 10Hz control rate aliases a real, sustained low-amplitude oscillation into the Logs view — dozens of lines a minute reporting a signal that was never actually unstable (occupancy and standing latency were unaffected throughout). The monitor now smooths its own 5-second samples with an EMA before comparing, raises the threshold to 10 ppm, and adds a 30-second floor between routine reports, while still reporting the instant the trim reaches the ±200 ppm clamp — the one signal that does mean something is wrong.
